...
public:
	...
	void Scale(qreal viewFactor, const QPointF& pinPoint) override
	{
	} // not necessary
...
private:
	...
	void UpdateRotate(qreal angle)
	{
		mTransformation.UpdateRotation(angle);
	};
	void UpdateTranslate(const QPointF& viewTranslation)
	{
		mTransformation.UpdateTranslation(viewTranslation);
	}
	void UpdateScale(qreal viewFactor)
	{
		mTransformation.UpdateScale(viewFactor);
	}
	QPointF Transform(const QPointF& point)
	{
		return mTransformation.Transform(point);
	}
